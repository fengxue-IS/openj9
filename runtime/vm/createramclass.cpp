...
{
	UDATA *vTableMethods = J9VTABLE_FROM_HEADER(vTableAddress);
	J9ROMClass *interfaceROMClass = interfaceClass->romClass;
	UDATA count = interfaceROMClass->romMethodCount;
	if (0 != count) {
				...
				while (tempIndex > 0) {
					...
					if (areNamesAndSignaturesEqual(interfaceMethodNameUTF, interfaceMethodSigUTF, vTableMethodNameUTF, vTableMethodSigUTF)) {
						...
						case SLOT_IS_CONFLICT_METHOD:
							 ...
							 */
							vTableMethods[tempIndex] = (UDATA)interfaceMethod;
							...
							if (((J9ITable *)methodClass->iTable)->depth < interfaceDepth) {
								...
								if (isSameOrSuperInterfaceOf(interfaceClass, methodClass)) {
									...
									if (J9_ARE_ANY_BITS_SET(combinedModifiers, J9_JAVA_ABSTRACT)) {
										...
										entry->method = (J9Method *)vTableMethods[tempIndex];
										entry->next = NULL;
										vTableMethods[tempIndex] = (UDATA)entry | EQUIVALENT_SET_ID_TAG;
										...
										*equivSetCount += 1;
										...
										/* Conflict detected */
										vTableMethods[tempIndex] |= DEFAULT_CONFLICT_METHOD_ID_TAG;
...
add_existing:
				...
				vTableMethodCount = growNewVTableSlot(vTableMethods, vTableMethodCount, interfaceMethod);
...
processVTableMethod(J9VMThread *vmThread, J9ClassLoader *classLoader, UDATA *vTableAddress, J9Class *superclass, J9ROMClass *romClass, J9ROMMethod *romMethod, UDATA localPackageID, UDATA vTableMethodCount, void *storeValue, J9OverrideErrorData *errorData)
{
	...
	if (J9_ARE_NO_BITS_SET(newModifiers, J9_JAVA_PRIVATE)) {
		...
		if (NULL != superclass) {
			...
			UDATA *superclassVTableMethods = J9VTABLE_FROM_HEADER(superclassVTable);
					...
					superclassVTableIndex)) != (UDATA)-1)
			{
				...
				do {
					...
					UDATA *currentSuperVTableMethods = J9VTABLE_FROM_HEADER(currentSuperclassVTable);
					J9Method *currentSuperclassVTableMethod = (J9Method *)currentSuperVTableMethods[superclassVTableIndex];
		...
		if (newSlotRequired) {
...
#if defined(J9VM_TRACE_VTABLE_ACCESS)
			{
				PORT_ACCESS_FROM_VMC(vmThread);
				J9UTF8 *classNameUTF = J9ROMCLASS_CLASSNAME(romClass);
				j9tty_printf(PORTLIB, "\n<vtbl_init: adding @ index=%d %.*s.%.*s%.*s (0x%x)>", vTableMethodCount - 1, J9UTF8_LENGTH(classNameUTF),
...
getVTableIndexForMethod(J9Method * method, J9Class *clazz, J9VMThread *vmThread)
{
	...
	if ((modifiers & J9_JAVA_INTERFACE) == J9_JAVA_INTERFACE) {
		...
		if (vTableSize == 0) {
			...
			vTableIndex = getVTableIndexForNameAndSigStartingAt(J9VTABLE_FROM_HEADER(vTable), nameUTF, sigUTF, vTableSize);
		...
		UDATA *vTableMethods = J9VTABLE_FROM_HEADER(vTable);
		UDATA vTableIndex = vTable->size;

		while (vTableIndex > 0) {
			vTableIndex--;
			if (method == (J9Method *)vTableMethods[vTableIndex]) {
				return J9VTABLE_OFFSET_FROM_INDEX(vTableIndex);
			}
		}
	}
