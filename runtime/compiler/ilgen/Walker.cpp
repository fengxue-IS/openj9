...
TR_J9ByteCodeIlGenerator::loadFromSideTableForInvokeDynamic(int32_t callSiteIndex)
   ...
   if (callSiteTableEntrySymRef->isUnresolved())
      ...
      TR::SymbolReference * appendixSymRef = symRefTab()->refineInvokeCacheElementSymRefWithKnownObjectIndexForInvokeDynamic(_methodSymbol, appendixNode->getSymbolReference(), callSiteIndex);
...
TR_J9ByteCodeIlGenerator::loadFromSideTableForInvokeHandle(int32_t cpIndex)
   ...
   if (methodTypeTableEntrySymRef->isUnresolved())
      ...
      {
      TR::Node * appendixNode = _stack->top();
      TR::SymbolReference * appendixSymRef = symRefTab()->refineInvokeCacheElementSymRefWithKnownObjectIndexForInvokeHandle(_methodSymbol, appendixNode->getSymbolReference(), cpIndex);
      appendixNode->setSymbolReference(appendixSymRef);
      }
   }
#endif
