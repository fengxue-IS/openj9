...
ifelse(LOOM_SUPPORT, ON, [	unsigned int can_support_virtual_threads : 1;], [	unsigned int : 16;])
	unsigned int : 16;
	unsigned int : 16;
	unsigned int : 16;
	unsigned int : 16;
...
ifelse(LOOM_SUPPORT, ON, [	JVMTI_EVENT_VIRTUAL_THREAD_START = 87,], [dnl])
ifelse(LOOM_SUPPORT, ON, [	JVMTI_EVENT_VIRTUAL_THREAD_END = 88,], [dnl])
...
ifelse(eval(JAVA_SPEC_VERSION >= 9), 1, [	jvmtiError (JNICALL * AddModuleReads)(jvmtiEnv* env, jobject module, jobject to_module);
	...
	void *reserved117;
ifelse(LOOM_SUPPORT, ON, [	jvmtiError (JNICALL * SuspendAllVirtualThreads)(jvmtiEnv* env, jint except_count, const jthread* except_list);
...
ifelse(LOOM_SUPPORT, ON, [	jvmtiError SuspendAllVirtualThreads (jint except_count, const jthread* except_list) { return functions->SuspendAllVirtualThreads(this, except_count, except_list); }
